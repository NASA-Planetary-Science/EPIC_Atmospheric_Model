...
c Read input fields of T,fp.
      ilatmax = 6
      ipmax   = 14
      open(unit=1,file='t_p_lt.s43',status='old')
      read(1,*)
     +  ((xlat(ilat),xp(ip), t(ilat,ip),ilat=1,ilatmax),ip=1,ipmax)
      close(1)
      open(unit=1,file='fp_p_lt.s43',status='old')
      read(1,*)
...
c Write output p[mbar], lat[deg]:
      ...
      write(3,'(1x,a9,14(1x,f6.2))') ' p[mbar]:',
     +         (1000.*exp(pln(i)),i=1,ipmax)
      write(3,*) 'lat[deg]'
      do ilat=1,ilatmax
        write(3,'(1x,f7.1,2x,14(1x,f6.4))') 

                  ...
                  double       *utaupr)
{
  ...
  if(ds->flag.ibcnd == GENERAL_BC && ds->bc.fbeam > 0.) {
	...
	/* Need Chapman function at top (0.0) and middle (0.5) of layer */
        CHTAU(lc)  = c_chapman(lc,0.0,taucpr,ds->nlyr,ds->zd,dtaucpr,zenang,ds->radius);
			       ...
			       double       *zbeama)
{
  ...
  rmin = 1.0e-4;
  if ( rcond < rmin ) {
    /*     Dither alpha if rcond too small  */
              ...
              double         radius)
{
  ...
  /*
   * T. Dowling, 7/19/13: Changed dithering value from DBL_EPSILON to dither as declared above.
   */
  for (lc = 1; lc <= ds->nlyr; lc++) {
    if (SSALB(lc) > 1.-dither) {
      SSALB(lc) = 1.-dither;
    }
    TAUC(lc) = TAUC(lc-1)+DTAUC(lc);
  }
...
void c_disort_state_alloc(disort_state *ds)
{
  ...
  /* 
   * Allocate ds->zd even if ds->flag.spher is currently not true, to avoid a segmentation fault
   * if ds->flag.spher is set to be true later.
   */ 
...
void c_twostr_state_alloc(disort_state *ds)
{
  ...
  /*
   * Allocate ds->zd, even if ds->flag.spher is currently not true, to
   * avoid a segmentation fault if ds->flag.spher is later turned on.
   */
  ds->zd = c_dbl_vector(0,ds->nlyr+1,"ds->zd");	

...
{
  static int
    self_tested = 1;   /* Set self_tested to -1 to enable self test, 1 to disable self test */
  int
    prntu0[2],
  ...
  for (mazim = 0; mazim <= naz; mazim++) {
	...
	for (lu = 1; lu <= ds->ntau; lu++) 
      ...
      /*
       * 30 June 2014: Changed from (kconv >= 2) to (kconv >= 4)
       *               following a libradtran development-team suggestion.
       */
      if (kconv >= 4) {
        break;
      }

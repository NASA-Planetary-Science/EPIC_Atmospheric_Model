/*
 *   C-language version of DISORT 2.1
 *   Copyright (c) 2011-2013 by Timothy E. Dowling
 *   
 ...
 *
 *     BDE: Buras R, Dowling T, Emde C, 2011, New secondary-scattering correction in DISORT with increased
 *              efficiency for foward scattering, J. Quant. Spect. Rad. Trans. 112, 2028-2034.
 ...
 *
 *   c_fcmp().....................Safe floating-point comparison function (more reliable than ==, but slower)
				...
				double        *utaupr)
{
  ...
  if (need_secondary_scattering == TRUE) {
	...
	for (lc = 1; lc <= ds->nlyr; lc++) {
      ...
      /* 
       * Calculate difference between original and delta-scaled phase functions (phas2)
       */
      for (lu = 1; lu <= ds->ntau; lu++) {
	PHAS2(it,lu) = 0.;
	...
	f_phas2 += (ds->MUP(it)-ds->MUP(it-1))*.5*(PHAS2(it,lu)+PHAS2(it-1,lu));
      }
  ...
  for (iu = 1; iu <= ds->numu; iu++) {
    ...
    for (jp = 1; jp <= ds->nphi; jp++) {
      ...
      for (lc = 1; lc <= ncut; lc++)
	PHASA(lc) = DSPHASE(it,lc)+ (ctheta-ds->MUP(it))
	...
	for (lc=1; lc <= ncut; lc++) {
      ...
      if (UMU(iu) < 0. && fabs(theta0-thetap) <= dtheta) {
        ...
        for (lu = ltau; lu <= ds->ntau; lu++) {
          if(!lyrcut || LAYRU(lu) < ncut) {
            duims         = c_new_secondary_scat(ds,iu,lu,it,ctheta,flyr,LAYRU(lu),tauc,nf,phas2, mu_eq, neg_phas,NORM_PHAS(lu));
	    UU(iu,lu,jp) -= duims;
          }
        }
      } 
			      ...
			      double       *norm_phas)
{
  ...
  for (lu = 1; lu <= ntau; lu++) {
    ...
    for (it = 2; it <= nphase; it++) {
      F_PHAS2_ABS(it) = F_PHAS2_ABS(it-1)+(MUP(it)-MUP(it-1))*.5*
	...
	it++;
      ...
      MU_EQ(i,lu) = MUP(it)+(f_phas2-F_PHAS2_ABS(it))
                  /(F_PHAS2_ABS(it+1)-F_PHAS2_ABS(it))
                  *(MUP(it+1)-MUP(it));

      if (PHAS2(it,lu) > 0. && PHAS2(it+1,lu) > 0.) {
	NEG_PHAS(i,lu) = FALSE;
      }
      else {
	if ( PHAS2(it,lu) < 0. && PHAS2(it+1,lu) < 0.) {
		    ...
		    int           callnum)
{
  ...
  if (ds->flag.ibcnd == GENERAL_BC) {
    if (ds->flag.usrtau) {
      ...
      for (lu = 1; lu <= ds->ntau; lu++) {
        /* Righthand-side was 1.e-4; modification from Libradtran known-bugs page */
...
/*
 ...
 C rewrite by Timothy E. Dowling (Univ. of Louisville), 2011-2013
 Fast, no-scattering cases added; version number incremented to C1.2, 2013.
              ...
              double         radius)
{
  ...
  if(ds->flag.prnt[0]) {
    fprintf(stdout,"\n\n\n\n"
            " ************************************************************************************************************************\n"
            "                         Two stream method radiative transfer program, version C1.2\n"
            " ************************************************************************************************************************\n");
  }

  /* 
   * Allocate memory for TAUC(lev)
   */
  tauc = c_dbl_vector(0,ds->nlyr,"tauc");
  ...
  /*
   * c_twostr_check_inputs(): Check input dimensions and variables.
   *
   * Return value: FALSE, no scattering; SSALB is nonpositive everywhere in the vertical column
   *               TRUE, scattering; SSALB is positive somewhere in the vertical column.
   */
  memset(ierror,0,TWOSTR_NERR*sizeof(int));
  scat_yes = c_twostr_check_inputs(ds,gg,ierror,tauc);
  ...
  for (ierr = 1; ierr <= TWOSTR_NERR; ierr++) {
    if (IERROR(ierr) != 0) {
      iret = 1;
      if (ds->flag.quiet==VERBOSE) {
        fprintf(stderr,"\nc_twostr_check_inputs reports fatal error: %d\n",ierr);
      }
    }
  }
  if (iret == 1) {
    /* 
     * Fatal input error detected.
     * Free allocated memory for TAUC(lev)
     */
    free(tauc);
    return;
  }
  ...
  if (scat_yes) {
    /*
     * Allocate memory for general scattering case
     */
    b       = c_dbl_vector(0,ds->nstr*ds->nlyr-1,"b");
    cband   = c_dbl_vector(0,ds->nstr*ds->nlyr*(9*(ds->nstr/2)-2)-1,"cband");
    ch      = c_dbl_vector(0,ds->nlyr-1,"ch");
    ...
    /*
     * Allocate memory for non-scattering case.
     */
    ch   = c_dbl_vector(0,ds->nlyr-1,"ch");
    pkag = c_dbl_vector(0,ds->nlyr,"pkag");
  }
  ...
  c_twostr_set(ds,&bplanck,ch,chtau,&cmu,deltam,dtaucpr,expbea,flyr,gg,ggprim,layru,&lyrcut,
               &ncut,&nn,oprim,pkag,pkagc,radius,tauc,taucpr,&tplanck,utaupr,scat_yes);

  /*
   * Print input information
   */
  if (ds->flag.prnt[0]) {
    c_twostr_print_inputs(ds,deltam,flyr,gg,lyrcut,oprim,tauc,taucpr,scat_yes);
  }
  ...
  if (scat_yes) {
    ...
    /*
     * Free memory allocated for scattering case.
     */
    free(b),     free(cband), free(ch),    free(chtau),free(diag), free(dtaucpr),free(expbea),free(flyr);
    free(fl),    free(ggprim),free(kk),    free(ll),   free(oprim),free(pkag),   free(pkagc), free(rr);
    free(taucpr),free(u0c),   free(utaupr),free(ts);
  }
  else {
    /*
     * Non-scattering case: compute upward and downward fluxes, mean intensities and flux divergences.
     */
    c_twostr_nonscat(ds,ch,layru,lyrcut,ncut,pkag,tplanck,bplanck,tauc,out);

    /*
     * Free memory allocated for non-scattering case.
     */
    free(ch),free(pkag);
  }
 ...
 *
 * Return value: FALSE, no scattering; SSALB = 0. everywhere in the vertical column
 *               TRUE,  scattering; SSALB > 0. somewhere in the vertical column
 */

int c_twostr_check_inputs(disort_state *ds,
                          double       *gg,
                          int          *ierror,
                          double       *tauc)
{
                     ...
                     disort_pair   *fl)
{
  ...
  /*
   * Zero disort_radiant-structure output array.
   */
  memset(out->rad,0,ds->ntau*sizeof(disort_radiant));
                           ...
                           int           scat_yes)
{
  ...
  if (scat_yes) {
    ...
    if (ds->flag.planck) {
      ...
      for (lc = 1; lc <= ds->nlyr; lc++) {
        fprintf(stdout,"%4d%10.4f%10.4f%10.5f%14.3f\n",
...
/*
 ...
 Input :  ds         'Disort' input variables
          scat_yes   TRUE for scattering case, FALSE for non-scattering case
...
/*
 * Calculates the radiative fluxes, mean intensity, and flux derivative with
 * respect to optical depth using the two-stream approximation, for the special
 * case of no scattering.  Uses eqns. (2.104, 2.106) from
 *
 *   Goody and Yung, 1989, Atmospheric Radiation: Theoretical Basis, 2nd Ed., Oxford Press
 *
 * Each integrand function, J(E_3) or J(E_2), is integrated using c_compact_integration(),
 * which implements the scheme of Leslie and Purser (1992, Meteorol. Atmos. Phys. 50, 61-73).
 *
 * T. Dowling, 2013
 */
...
/*
 ...
 I n p u t     v a r i a b l e s:
   ...
   lyrcut        :  Logical flag for truncation of comput. layer
   ncut          :  Number of computational layer where absorption optical depth exceeds -abscut-
   pkag          :  Integrated Planck function for internal emission at layer boundaries
   tplanck       :  Top boundary thermal emission
   bplanck       :  Bottom boundary thermal emission
   tauc          :  Cumulative optical depth at bottom of layers
 ...
 I n t e r n a l     v a r i a b l e s:
   ...
   fnet          :  net flux (total-down - diffuse-up)
   fact          :  EXP(-utaupr/ch), where ch is the Chapman factor
   planck_source :  Planck source function (thermal)
   c_J_E_ndat    :  Number of data points in table, which varies with position of UTAU(lu)
   c_J_E_table   :  Table entries for J vs. E, following eqns. (2.104, 2.106) of Goody and Yung (1989)

 ---------------------------------------------------------------------*/

/*
 * DTAUDIFF_MIN: Minimum differential in numerical integrations. Should be at least DBL_EPSILON.
 *               Setting this greater than DBL_EPSILON, but not too large, speeds up the calculations
 *               without compromising the accuracy.
 * TAUDIFF_MAX:  Maximum relative optical depth for which calculations are performed.
 */
#define DTAUDIFF_MIN 1.e-7
#define TAUDIFF_MAX  10.

void c_twostr_nonscat(disort_state  *ds,
                      double        *ch,
                      int           *layru,
                      int            lyrcut,
                      int            ncut,
                      ...
                      disort_output *out)
{
  int
    ...
    c_J_E_ndat;
  double
    temp,taudiff,taudiff_old,dtaudiff,dtau,
    fdntot,fnet,planck_source,dirint;
  dbl_triplet
   *t_table,
   *c_J_E2_table,
   *c_J_E3_table;
  ...
  if (ds->flag.planck) {
    /* Allocate memory */
    t_table      = c_dbl_triplet(0,ds->nlyr,"t_table");
    c_J_E2_table = c_dbl_triplet(0,ds->nlyr,"c_J_E2_table");
    c_J_E3_table = c_dbl_triplet(0,ds->nlyr,"c_J_E3_table");

    /* 
     * Set up spline of temperature as a function of optical depth,
     * TEMPER(lev) vs. TAUC(lev).
     */
    for (lev = 0; lev <= ds->nlyr; lev++) {
      t_table[lev].x = TAUC(lev);
      t_table[lev].y = TEMPER(lev);
    }
    c_spline_pchip(ds->nlyr+1,t_table);

    /* Initialize temperature-table index for c_hunt_place_in_table() */
    iplanck = -2;
  }
   ...
   */
  for (lu = 1; lu <= ds->ntau; lu++) {
    lyu = LAYRU(lu);  /* lyu is the layer that contains UTAU */
    ...
    if (lyrcut && lyu > ncut) {
      ...
      if (ds->bc.fbeam > 0.) {
        if (ds->bc.umu0 > 0. || ds->flag.spher) {
           ...
           */
          dirint     = ds->bc.fbeam*exp(-UTAU(lu)/CH(lyu));
      ...
      if (ds->flag.planck) {
        /*
         * Include internal-source thermal radiation (typically the longwave, planetary component).
         */
        iplanck       = c_hunt_place_in_table(ds->nlyr+1,t_table,UTAU(lu),&dtau,iplanck);
        temp          = c_splint_pchip(UTAU(lu),t_table+iplanck,dtau);
        planck_source = c_planck_func2(ds->wvnmlo,ds->wvnmhi,temp);

        /* 
         * Calculate upward intensity and diffuse flux.
         *
         * Specify table values for c_compact_integration().
         * Implementation of eqns. (2.106) and (2.104) of Goody and Yung (1989, p. 49).
         */
        i           = 0;
        taudiff_old = TAUDIFF_MAX;
        for (lev = ds->nlyr; lev >= 0; lev--) {
          taudiff  = TAUC(lev)-UTAU(lu);

          if (taudiff > TAUDIFF_MAX) {
            /*
             * Too far from UTAU to make a significant contribution.
             */
            continue;
          }

          if (taudiff <= 1.5*DTAUDIFF_MIN) {
            /*
             * Last table entries are at UTAU.
             */
            c_J_E2_table[i].x  = 1.;
            c_J_E2_table[i].y  = planck_source;

            c_J_E3_table[i].x  = .5;
            c_J_E3_table[i].y  = planck_source;

            i++;
            break;
          }

          dtaudiff = taudiff_old-taudiff;
          if (dtaudiff >= DTAUDIFF_MIN) {
            /*
             * Regular table entries.
             */
            c_J_E2_table[i].x  = c_exp_integral(2,taudiff);
            c_J_E2_table[i].y  = PKAG(lev);

            c_J_E3_table[i].x  = c_exp_integral(3,taudiff);
            c_J_E3_table[i].y  = PKAG(lev);
         ...
         */
        c_J_E2_table[0].z = 0.;
        c_compact_integration(c_J_E_ndat,c_J_E2_table);
        UAVG(lu) = .5*(bplanck*c_J_E2_table[0].x+c_J_E2_table[c_J_E_ndat-1].z);

        c_J_E3_table[0].z = 0.;
        c_compact_integration(c_J_E_ndat,c_J_E3_table);
        FLUP(lu) = 2.*M_PI*(bplanck*c_J_E3_table[0].x+c_J_E3_table[c_J_E_ndat-1].z);

        /* 
         * Calculate downward diffuse flux, and downward component of intensity.
         *
         * Specify table values for spline.
         * Implementation of eqn. (2.106) of Goody and Yung (1989, p. 49).
         */
        i           = 0;
        taudiff_old = TAUDIFF_MAX;
        for (lev = 0; lev <= ds->nlyr; lev++) {
          taudiff = UTAU(lu)-TAUC(lev);
        ...
        c_J_E2_table[0].z = 0.;
        c_compact_integration(c_J_E_ndat,c_J_E2_table);
        UAVG(lu) += .5*((ds->bc.fisot+tplanck)*c_J_E2_table[0].x+c_J_E2_table[c_J_E_ndat-1].z);

        c_J_E3_table[0].z = 0.;
        c_compact_integration(c_J_E_ndat,c_J_E3_table);
        RFLDN(lu) = 2.*M_PI*((ds->bc.fisot+tplanck)*c_J_E3_table[0].x+c_J_E3_table[c_J_E_ndat-1].z);
      }
      else {
        planck_source = 0.;
      }

      fdntot = RFLDN(lu)+RFLDIR(lu);
      fnet   = fdntot-FLUP(lu);

      /*
       * Add direct-beam intensity to get the total mean intensity.
       */
      UAVG(lu) += dirint/(4.*M_PI);

      /*
       * Flux divergence, not calculated as a finite difference but analytically as in KST(12),
       * here with single-scattering albedo set to zero.
       */
      DFDT(lu) = 4.*M_PI*(UAVG(lu)-planck_source);
    }

    if (ds->flag.prnt[1]) {
      fprintf(stdout,"%10.4f%7d%12.3e%12.3e%12.3e%12.3e%12.3e%12.3e%12.3e%14.3e\n",
                     UTAU(lu),lyu,RFLDIR(lu),RFLDN(lu),fdntot,FLUP(lu),fnet,UAVG(lu),planck_source,DFDT(lu));
    }
  }  /* end of lu loop */
...
/*============================= end of c_twostr_nonscat() ================*/

/*============================= c_compact_integration() ==================*/

/*
 * 3rd-order accurate integration of y = dz/dx on a non-staggered grid with
 * variable grid spacing (endpoints are 2nd-order accurate).
 *
 * Input:
 * table[0 to n-1].x: 	independent-variable, x
 * table[0 to n-1].y: 	derivative, y = dz/dx
 * table[0].z:		value of integral at starting position, z0
 *
 * Output:
 * table[0 to n-1].z: 	the integral of dz/dx
 *
 ...
 *
 * Adapted by T. Dowling, 2013
 */
void c_compact_integration(int          n,
                           dbl_triplet *table)
{
  int
    k;
  double
    dx30,dx31,dx32,dx20,dx21,dx10,
  ...
  /*  Centered, trapezoidal step from start. */
  k            = 0;
  dz           = .5*(table[k].y+table[k+1].y)*(table[k+1].x-table[k].x);
  table[k+1].z = table[k].z+dz;
  ...
  if (n > 3) {
    for (k = 1; k < n-2; k++) {
      dx10  = table[k  ].x-table[k-1].x;
      dx21  = table[k+1].x-table[k  ].x;
      dx32  = table[k+2].x-table[k+1].x;
      dx20  = dx21+dx10;
      dx30  = dx32+dx20;
      dx31  = dx32+dx21;
      dzdx0 = table[k-1].y;
      ...
      dzdx2 = table[k+1].y;
      dzdx3 = table[k+2].y;
      dz    = -(dzdx0*((dx21/dx10)*(dx21/dx20)*(dx21/dx30)*(dx31+dx32))
               +dzdx1*((dx21/dx10)*((dx21/dx31)*(dx21+2.*(dx10-dx31))-6.*dx10))
               +dzdx2*((dx21/dx20)*((dx21/dx32)*(dx21+2.*(dx32-dx20))-6.*dx20))
               +dzdx3*((dx21/dx30)*(dx21/dx31)*(dx21/dx32)*(dx10+dx20))        )/12.;

      table[k+1].z = table[k].z+dz;
    }
  }

  /*  Centered, trapezoidal step to end. */
  k            = n-2;
  dz           = .5*(table[k].y+table[k+1].y)*(table[k+1].x-table[k].x);
  table[k+1].z = table[k].z+dz;
                     ...
                     char *name)
{
  ...
  if (!m) {
    c_errmsg("c_dbl_vector---alloc error",DS_ERROR);
  }
  m -= nl_safe;
...
/*
 * Frees memory allocated by c_dbl_vector().
 *
 * NOTE: If the array is zero-offset, can just use free().
 * NOTE: Argument nh is not used, but kept to match c_dbl_vector().
 */
                  ...
                  char *name)
{
  ...
  if (!m) {
    c_errmsg("c_int_vector---alloc error",DS_ERROR);
  }
  m -= nl_safe;
...
/*
 * Frees memory allocated by c_int_vector().
 *
 * NOTE: If the array is zero-offset, can just use free().
 * NOTE: Argument nh is not used, but kept to match c_dbl_vector().
 */

void c_free_int_vector(int *m, 
                       int  nl, 
                       int  nh)
{
  int  
...
/*============================= end of c_free_int_vector() ==============*/

/*============================= c_dbl_triplet() =========================*/
/*
 * Allocates memory for a 1D dbl_triplet array with range [nl..nh].
 *
 * NOTE: calloc() zeros the memory it allocates.
 */ 

dbl_triplet *c_dbl_triplet(int   nl, 
                           int   nh,
                           char *name)
{
  unsigned int  
  ...
  m = (dbl_triplet *)calloc(len_safe,sizeof(dbl_triplet));

  if (!m) {
    c_errmsg("c_dbl_triplet---alloc error",DS_ERROR);
  }
  m -= nl_safe;
...
/*============================= end of c_dbl_triplet() ==================*/

/*============================= c_free_dbl_triplet() ====================*/

/*
 * Frees memory allocated by c_dbl_triplet().
 *
 * NOTE: If the array is zero-offset, can just use free().
 * NOTE: Argument nh is not used, but kept to match c_dbl_triplet().
 */

void c_free_dbl_triplet(dbl_triplet *m,
                        int          nl,
                        int          nh)
{
  int  
...
/*============================= end of c_free_dbl_triplet() =============*/

/*============================= c_hunt_place_in_table() =================*/

/*
 * Find place in table using bisection.
 * Adapted by Michael Sussman for the EPIC atmospheric model,
 * from Numerical Recipes in C, p. 118-119.
 * Incorporated into CDISORT by Tim Dowling, 2013.
 *
 * NOTE: Hunts place relative to the x component of the dbl_triplet table.
 ...
 */
int c_hunt_place_in_table(int          n,
                          dbl_triplet *table,
                          double       x,
                          double      *dx,
                          int          il)
{
  ...
  cmplo  = c_fcmp(x,table[0  ].x);
  cmphi  = c_fcmp(x,table[n-1].x);
 ...
 *
 *    Adapted to CDISORT by Tim Dowling, 2013.
 *
 *  Reference:
 *
                    ...
                    dbl_triplet *table)
{
   ...
   */
  if (n < 2) {
    fprintf(stderr,"Error: c_spline_pchip: no. data points, n=%d, is less than 2\n",n);
    exit(1);
  }

  for (i = 1; i < n; i++) {
    if (table[i].x <= table[i-1].x) {
      fprintf(stderr,"ERROR: c_spline_pchip: x array is not strictly increasing, table[%d].x=%g <= table[%d].x=%g\n",
  ...
  /*
   * Set table[0].z via non-centered three point formula,
  ...
  if (c_pchst(table[0].z,del1) <= 0.) {
    table[0].z = 0.0;
  }
  else if (c_pchst(del1,del2) < 0.) {
    /*
  ...
  /*
   *  Set table[n-1].z via non-centered three point formula,
   *  adjusted to be shape preserving.
   */
  w1           = -h2/hsum;
  w2           = (h2+hsum)/hsum;
 ...
 *    is designed to work with c_spline_pchip().
 *
 ...
 *
 *    Input, h = dx, the x interval, as returned by c_hunt_place_in_table().
 *
 *    Returns the value of the cubic function at the point xx.
 */

double c_splint_pchip(double       xx,
                      dbl_triplet *table,
                      double       h)
{
  register double 
...
/*============================= end of c_splint_pchip() =================*/

/*============================= c_linint() ==============================*/

/*
 * Evaluates linear interpolation using same arguments as c_splint_pchip(),
 * to make it easy to switch between the two.
 */

double c_linint(double       xx,
                dbl_triplet *table,
                double       h)
{
  double
  ...
  ans = table[0].y+(table[1].y-table[0].y)*(xx-table[0].x)/h;
...
/*
 * Compute the exponential-integral function E_n(x).
 *
 * Adapted by Tim Dowling, 2013, from Numerical Recipes in C, 2nd Ed, p. 223-224.
 */
                      ...
                      double x)
{
  ...
  /* Print error message and exit if bad input is detected. */
  if (n < 0 || x < 0. || (x == 0. && (n == 0 || n == 1))) {
    fprintf(stderr,"Error: c_exp_integral: bad argument(s), n=%d x=%g\n",
                    n,x);
    exit(1);
  }
  ...
  if (n == 0) {
    ...
    if (x == 0.) {
      ...
      if (x > 1.) {
        ...
        fprintf(stderr,"Error: c_exp_integral(): continued fraction failed with MAX_ITER=%d\n",
                       MAX_ITER);
        exit(1);
      }
      else {
        /* Evaluate series.  Set first terms */
        ...
        fprintf(stderr,"Error: c_exp_integral(): series failed with MAX_ITER=%d\n",
                       MAX_ITER);
        exit(1);
      }
    }
...
/*============================= end of c_exp_integral() =================*/

/*============================= c_table_exp_integral() ==================*/

/*
 * Evaluate exponential integrals via cubic splines.
 *
 * T. Dowling, 2013
 */

/*
 * If n > MAX_ORDER is desired, increase MAX_ORDER here,
 * and add the case to the switch(n) below.
 *
 * Table resolution can be increased at the expense of 
 * table look-up speed by increasing NDAT.
 *
 * NOTE: This function is only a few percent faster than c_exp_integral() itself.
 */
#define MAX_ORDER 5
#define NDAT      501
#define XTOP      11.
...
#define SPLINE_MACRO \
    {\
      ...
      if (!initialized) { \
        /* Allocate memory */ \
        table[n-1] = (dbl_triplet *)calloc(NDAT,sizeof(dbl_triplet)); \
        if (!table[n-1]) { \
          fprintf(stderr,"Error: c_table_exp_integral: could not allocate table for n=%d\n",n); \
          exit(1); \
        } \
        /* Set up table */ \
        for (i = 0; i < NDAT; i++) { \
          table[n-1][i].x = xx = XTOP*(double)(i)/(NDAT-1); \
          table[n-1][i].y = c_exp_integral(n,xx); \
        } \
        /* Set up spline */ \
        c_spline_pchip(NDAT,table[n-1]); \
        initialized = TRUE; \
      } \
      /* Spline interpolation */ \
      itable = c_hunt_place_in_table(NDAT,table[n-1],x,&dx,itable); \
      ans    = c_splint_pchip(x,table[n-1]+itable,dx); \
    }

double c_table_exp_integral(int    n,
                            double x)
{
  int
    i;
  ...
  if (!initialized) {
     ...
     * Allocate memory
     */
    table = (dbl_triplet **)calloc(MAX_ORDER,sizeof(dbl_triplet *));
    if (!table) {
      fprintf(stderr,"Error: c_table_exp_integral: could not allocate table\n");
      exit(1);
    }
  ...
  if (n == 0) {
    ...
    /* Return asymptotic-expansion value */
    ans = (1.-((double)n/x)*(1.-(double)(n+1)/x))*exp(-x)/x;
  }
  else {
    switch(n) {
      case 1: SPLINE_MACRO; break;
      case 2: SPLINE_MACRO; break;
      case 3: SPLINE_MACRO; break;
      case 4: SPLINE_MACRO; break;
      case 5: SPLINE_MACRO; break;
      default:
       fprintf(stderr,"Error: c_table_exp_integral: n=%d not yet implemented\n",n);
       exit(1);
      break;
    }
  }
                               ...
                               double umu0)
{
  ...
  for (iq = 1; iq <= nn; iq++) {
    if(fabs((umu0-CMU(iq))/umu0) < 1.e-4) {
      umu0s = umu0;
      if (umu0 < CMU(iq)) {
